...
export class Registrations implements OnInit {
  ...
  load(): void {
    ...
    const obs = this.auth.isAdmin ? this.regSvc.getAllRegistrations() : this.regSvc.getMyRegistrations();
    obs.subscribe({
      next: regs => { this.registrations = regs || []; this.applyFilter(); this.loading = false; },
      error: () => { this.error = 'Failed to load registrations.'; this.loading = false; this.registrations = []; this.filtered = []; }
    });
  }
